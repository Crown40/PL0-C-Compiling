		...
		i=position(id,*ptx);
   	 	...
   	 	else
   	 	{
   	 		...
   	 		else
   	 		{
   	 			...
   	 			int type=0;
   	 			/*Add_N_Alter*/
				if(sym==becomes||sym==plusassign||sym==minusassign)
   	 			{
					type=sym;
   	 				getsymdo;
						...
						else
						{
							...
							else
							{
								...
								if(sym==forsym){
									getsymdo;
									if(sym==ident){
										//enter(variable,ptx,lev+1,pdx);//level+1:填写名字表
										i=position(id, *ptx);
										if(i==0){
											error(51);//不存在的变量 
										}
											...
											if(sym==becomes){
												getsymdo; 
												memcpy(nxtlev,fsys,sizeof(bool)* symnum);
												expressiondo(nxtlev,ptx,lev);//Level 
												if(i!=0){
													gendo(sto,lev-table[i].level,table[i].adr); 
												} 
												//2nd
												...
												if(sym==tosym||sym==downtosym){
													...
													//
													if(type==tosym){
														gendo(opr,0,10);//<id>小于<表达式>	
													}
													else{
														gendo(opr,0,12);//<id>大于<表达式>
													}
													// 
													cx2=cx;
													gendo(jpc,0,0);
				...
				switch(table[i].kind)
				{
					...
					case variable:                                      /*名字为变量*/
						...
						if(sym==increment||sym==decrement){
							gendo(lod,lev-table[i].level,table[i].adr);
							int type=sym;
							while(sym==increment||sym==decrement){
								type=sym;
								if(type==increment){
									gendo(lit,0,1);
									gendo(opr,0,2);
